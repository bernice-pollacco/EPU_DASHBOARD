#BUILDING FUNCTIONS TO INPUT IN SHINYR
...
#load datasets
episodes_raw <- read_csv("EPU_Episodes_v2.csv")
lab_results_raw <- read_csv("EPU_Lab_Results_v2.csv")
staff_shifts_raw <- read_csv("EPU_Staffing_Shifts_v2.csv")


#inspect dataset
summary(episodes_raw)
...
#Bar graph showing the number of each diagnosis
...
counts_per_diagnosis_function(start_date, end_date)

#Percentage of people who DNA and Lost to Followup
...
dna_ltfu_function(start_date, end_date, "Ectopic Pregnancy")
 

#Percentage Emergency Surgery for Ectopics
#These will be displayed on Overview Page as numbers
 ...
 # episodes_with_date %>%
  #  filter(Outcome_Category=="Ectopic Pregnancy",
   #        Management_Type=="Surgical",
    ...
    #summarise(
     # percent_emergency_surgery = round(mean(em_surgery$Emergency_Surgery == "Yes") * 100, 2)) %>% 
    #pull(percent_emergency_surgery)
#}

percent_emergency_ectopic_surgery_function <- function(start_date, end_date){
  
  # Filter the data
  em_surgery <- episodes_with_date %>%
    filter(
  ...
  if(nrow(em_surgery) == 0){
    return(0)  # or NA, or "No data"
  }
  
  # Compute % of emergency surgeries
  percent <- mean(em_surgery$Emergency_Surgery == "Yes", na.rm = TRUE) * 100
...
avg_monthly_waiting_time_function <- function(start_date, end_date, diagnosis){
  episodes_with_date %>%
    filter(seen_date >= start_date & seen_date <= end_date,
           Outcome_Category==diagnosis) %>%
    mutate(
      waiting_time = as.numeric(difftime(Seen_in_Admission_Time, Arrival_Time, units = "hours")),
      Review_Month = as.Date(floor_date(Arrival_Time, "month")
      )) %>%
...
avg_monthly_waiting_time_function(start_date, end_date, "Ectopic Pregnancy")
##############################################################################
...
##############################################################################
#Average hours for HCG result
avg_hrs_hcg_monthly_function <- function(start_date, end_date, diagnosis){
  episodes_with_date %>%
    filter(seen_date >= start_date & seen_date <= end_date,
           Outcome_Category==diagnosis) %>%
    mutate(
      hcg_time = as.numeric(difftime(hCG_Result_Time, hCG_Sample_Time, units = "hours")),
      Sample_Month = as.Date(floor_date(hCG_Sample_Time, "month")
      )) %>%
    group_by(Sample_Month) %>%
    summarise(
      Avg_hcg_time = mean(hcg_time, na.rm = TRUE),
      Patients = n()
    )
} 
...
#Average time to diagnose Ectopics
...
management_type_barchart_function(start_date, end_date, "Late Miscarriage")
management_type_barchart_function(start_date, end_date, "Ectopic Pregnancy")
management_type_barchart_function(start_date, end_date, "PUL")
management_type_barchart_function(start_date, end_date, "IUD")
...
#Line graph with number of patients admitted over time
...
#Line graph with number of patients seen in admission room over time
...
monthly_emergency_rate_function(start_date, end_date, "Early Miscarriage")
...
#Function to create staff line chart based on user selection of shift type
...
#Function to create percentage pool line chart based on user selection of shift type
...
#relationship between wait time and diagnosis
rel_wait_diagnosis_function <- function(start_date, end_date){
  episodes_with_date %>%
    ...
    mutate(
      waiting_time = as.numeric(difftime(Seen_in_Admission_Time, Arrival_Time, units = "hours"))) %>% 
    ggplot(aes(x=Outcome_Category, y=waiting_time))+
    geom_boxplot()+
...
rel_wait_emsurgery_function <- function(start_date, end_date, diagnosis){
  episodes_with_date %>% 
    ...
    geom_boxplot()+
    labs(title = "Relationship between Emergency Surgery and Waiting Time (hrs)",
...
rel_wait_emsurgery_function(start_date, end_date, "PUL")
...
rel_los_diagnosis_function(start_date, end_date)
